$(document).ready(function () {
	var requiredErrorMessage = "下記内容は必須事項になります。";
	var emailPatternInvalidErrorMessage = "ローマ字、数字、＠、. のみでご記入お願いします。 (e.g: example@domain.com)";
	var phoneNumberPatternInvalidErrorMessage = "数字のみでご記入お願いします。 (e.g: 01012345678)";
	var clinicNotAvailableErrorMessage = "休診日になります。他の日程を選択してください。";
    ...
    var clinics = [
        ...
        },
        {
            ...
            address: "ソウル市 江南区 江南デロ470、808タワー 13〜14F",
        ...
        {
            name: "Ppeum Hongdae",
            address: "ソウル市 麻浦区 ヤンファロ 140 ナンガンビル 3F",
            supportLanguages: [""],
            hours: [
            { min: 1030, max: 1530 },
            { min: 1030, max: 1930 },
	...
	$(timeHourFiled).change(function() {
	  ...
	  if (hour != 12 && dayNight[1] == "午後") {
	    hour += 12;
	  }
	  if(hour == 12 && dayNight[1] == '午前') {
		hour = 0;
	  }
	...
	function initTimepicker($field, $minTime, $maxTime, $lunchStartTime, $lunchEndTime) {
      if ($minTime != null && $maxTime != null) {
		...
		$($field).timepicker({
		  ...
		  var temp = ((mer == "午後" && h < 12) ? ((h+12) + "" + m) : ((mer == "午前" && h >= 12) ? ((h-12) + "" + m) : (h + "" + m)));
