$(document).ready(function () {
	var requiredErrorMessage = "以下内容是必填事项";
	var emailPatternInvalidErrorMessage = "您提供的电子邮件地址不正确。电子邮件只能包含拉丁字母、数字、“@”和“.”（例如：example@domain.com）";
	var phoneNumberPatternInvalidErrorMessage = "您填写的电话号码不正确。电话号码仅包含数字（例如：01012345678）";
	var clinicNotAvailableErrorMessage = "医院的休息日，请选择其他日期";
    ...
    var clinics = [
        ...
        },
        {
            ...
            address: "首尔市瑞草区江南大路470 808大厦，13~14楼",
	...
	$(timeHourFiled).change(function() {
	  ...
	  if (hour != 12 && dayNight[1] == "下午") {
	    hour += 12;
	  }
	  if(hour == 12 && dayNight[1] == '上午') {
		hour = 0;
	  }
	...
	function initTimepicker($field, $minTime, $maxTime, $lunchStartTime, $lunchEndTime) {
      if ($minTime != null && $maxTime != null) {
		...
		$($field).timepicker({
		  ...
		  var temp = ((mer == "下午" && h < 12) ? ((h+12) + "" + m) : ((mer == "上午" && h >= 12) ? ((h-12) + "" + m) : (h + "" + m)));
