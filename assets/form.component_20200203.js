$(document).ready(function () {
	...
	var userNameBottomField = "#user-name-bottom";
	var userMessengerIdBottomField = "#user-messenger-id-bottom";
	var userMessengerTypeBottomField = "#user-messenger-type-bottom";
	var userEmailAddressBottomField = "#user-email-address-bottom";
	...
	$('#booking-form').submit(function(event){
		...
		else location.href = "https://beauty.jivaka.care/pages/coupon-confirmation";

	});
	$('#booking-form-bottom').submit(function(event){
		if (!isFormBottomValid()) {
			event.preventDefault();  
		}
		else location.href = "https://beauty.jivaka.care/pages/coupon-confirmation";
	});
	...
	function isFormBottomValid() {
		var isValid = true;
		var errorField = "";
		if (!isRequiredFieldValid(userNameBottomField)) { errorField = setErrorField(isValid, userNameBottomField, errorField); isValid = false; }
		if (!isRequiredFieldValid(userMessengerIdBottomField)) { errorField = setErrorField(isValid, userNameBottomField, errorField); isValid = false; }
		if (!isRequiredFieldValid(userEmailAddressBottomField) || !isEmailBottomValid()) { errorField = setErrorField(isValid, userEmailAddressBottomField, errorField); isValid = false; }
	...
	function isEmailBottomValid() {
		var emailRegex = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
  
		return isPatternFieldValid(emailRegex, userEmailAddressBottomField, emailPatternInvalidErrorMessage);
	  }
